...
class UserController 
{
    ...
    static async create(req, res)
    {
        try {
            ...
            const findUsername = await UserService.findUsername(username);
            ...
            if(findEmail){
                return res.status(400).send('Email sudah di gunakan');
            }
    
            const hashedPassword = await bcrypt.hash(password, 10);
    
            users.push({ firstname, lastname, username, email, password: hashedPassword, address});
    ...
    static async login(req, res)
    {
        try {
            ...
            const checkAccount         = await UserService.findUsername(username);
            ...
            const token = JWT.generateToken({ email:checkAccount.email, username:checkAccount.username });
            res.json(
                { 
                    token,
                    user : {
